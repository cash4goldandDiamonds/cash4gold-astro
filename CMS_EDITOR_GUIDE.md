...
**The following workflow is implemented in code but has not passed the actual Sanity editor acceptance test.** Do not treat it as an active production editing service. The private `migration-staging` dataset is the first test destination. Production was observed empty.

## Edit an article

1. Open the existing Pages and articles document. Keep its current URL path. Choose a clear page title and author. Preserve the original publication date; use Actual content revision date only when the public content meaningfully changes.
