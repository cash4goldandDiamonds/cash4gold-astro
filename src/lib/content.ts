...
function configuredClient(){if(buildEnvironment.CONTENT_SOURCE==='reviewed-static'){assertContentEnvironment(buildEnvironment);return null;}const projectId=import.meta.env.SANITY_PROJECT_ID,dataset=import.meta.env.SANITY_DATASET;const policy=releasePolicy(buildEnvironment);if(!projectId&&!dataset)return null;if(!projectId||!dataset)throw new Error('Configure both Sanity project and dataset.');return createClient({projectId,dataset,apiVersion:import.meta.env.SANITY_API_VERSION||'2026-09-09',useCdn:false,token:import.meta.env.SANITY_READ_TOKEN,perspective:policy.perspective});}
