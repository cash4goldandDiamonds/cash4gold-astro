...
export function getPages():Promise<Page[]>{return cached??=(async()=>{
 ...
 const docs=await client.fetch('*[_type == "page"]{...,body[]{...,markDefs[]{...,target->{path}}},author->,related[]->{path,title},categories[]->{title},service->{title,"page":page->{path}},faqs[]->{question,answer}}');
 ...
 const pages=applyAddressCorrections([...result.values()]);assertAddressIntegrity(pages);return applyImageOverrides(pages) as Page[];
})();}
let redirectCache:Promise<{from:string,to:string,status:number}[]>|undefined;
export function getRedirects(){return redirectCache??=(async()=>{const client=configuredClient();const cms=client?await client.fetch('*[_type=="redirect"]{from,to,status}'):[];return resolveRedirects(localRedirects,cms,await getPages());})();}
