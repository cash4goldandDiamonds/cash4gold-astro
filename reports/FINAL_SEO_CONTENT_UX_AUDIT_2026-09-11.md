...
The first complete authenticated staging crawl found empty Open Graph URLs on all 74 archive routes, despite correct canonical tags. The shared layout now uses the same canonical fallback for Open Graph. The automated check now rejects missing or mismatched Open Graph URLs and missing social images on every content route. These archive routes keep their existing noindex policy. Final deployed verification is recorded in the handoff with the exact published commit.
