...
1. Complete the pending approval in the existing deployment task to inspect Cloudflare's GitHub connection permissions. That screen inspection is not approval for all repositories. The operator must then finish narrowly scoped staging setup, verify authentication before enabling any public staging endpoint, and run deployed checks. Do not enable account-wide Access settings or production routing to solve this blocker.
2. Sign back into the open GoDaddy tab. The operator still needs a complete downloaded WordPress database/files/uploads backup, authorized WordPress/Rank Math/media exports and an isolated restore. No hosting setting change is required for this sign-in.
3. Confirm the inquiry delivery provider and a test inbox/sandbox authorized to receive synthetic submissions. Confirm a safe scheduling test destination. The operator must implement the missing form, then verify actual delivery, errors, retries and appointment semantics.
...
6. Approve factual business details, hours/arrival instructions, media rights and the content difference ledger; reconcile 222 fetched versus 223 reported WordPress media records and the final content delta. Existing unapproved content has not become owner-approved merely because it built successfully.
